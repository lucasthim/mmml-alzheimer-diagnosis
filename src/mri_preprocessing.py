...
def execute_preprocessing(input_path,output_path):
    ...
    for ii,image_path in enumerate(images_to_process):
        ...
        image_to_process = register_image_with_atlas(input_path=image_path.as_posix())
        
        print("Stripping skull from image...")
        deep_brain_skull_stripping(input_path = input_path, output_path = output_path, input_img=image_to_process.numpy(), probability = 0.5)
        
        # TODO: Apply cropping based on bbox
        # print("Cropping image with bounding box.")
