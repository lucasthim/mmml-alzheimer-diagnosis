...
| Broken CLIs: crash when run as `__main__` | [src/data_preprocessing/mri_selection.py#L73](../../src/data_preprocessing/mri_selection.py#L73), [mri_metadata_preprocessing.py#L122](../../src/data_preprocessing/mri_metadata_preprocessing.py#L122) | `mri_selection` reads `args.cognitive_data_path` (the dest is `cognitive`) and mis-parses `--classes type=list`; `mri_metadata_preprocessing` never parses `args`. | Import and call `select_mris_to_download(...)` / the `execute_..._preprocessing_*` functions directly from a notebook/REPL. See [data-acquisition.md](../data/data-acquisition.md). |
| Dead `existing_reference_path` branch in selection | [src/data_preprocessing/mri_selection.py#L34](../../src/data_preprocessing/mri_selection.py#L34) | `filter_images` references an undefined `df_cog` and never returns. | Leave `existing_reference_path=None`. |
...
| Issue | Location | Impact | Suggested fix |
|---|---|---|---|
| Rotation/neighbor sampling is non-reproducible | [mri_augmentation.py#L87](../../src/data_preparation/mri_augmentation.py#L87), `:135`; [mri_metadata_preparation.py#L104](../../src/data_preparation/mri_metadata_preparation.py#L104), `:111` | You **cannot reproduce a previous augmented dataset** even with the same config — the splits are deterministic but the augmented slices/angles are not. Re-running prep yields a different training set. | Set a fixed seed (`random.seed(<fixed>)`) where reproducible augmentation matters. |
| Commented-out dead code with a copy/paste bug (`samples[0]` used thrice) | [mri_augmentation.py#L101](../../src/data_preparation/mri_augmentation.py#L101) (through `:111`) | An earlier rotation routine; harmless because it is commented out, but misleading if you uncomment it. | Delete it. |
| Recurring TODO: no bounds check on `slice ± sampling_range` vs volume size | multiple docstrings, e.g. [mri_preparation.py#L50](../../src/data_preparation/mri_preparation.py#L50), [mri_augmentation.py#L34](../../src/data_preparation/mri_augmentation.py#L34) | If `orientation_slice + sampling_range` exceeds the volume bounds it can index out of range. "Values range from 0 to 100" in the docstrings is the implicit contract. | Add a bounds check / clamp before sampling neighbors. |
...
| Issue | Location | Impact | Suggested fix |
|---|---|---|---|
| Two incompatible path roots: preprocessing **writes** to the nested `.../mmml-alzheimer-diagnosis/data/...`, everything else **reads** from `.../Lucas_Thimoteo/data/...` | [mri_preprocessing.py#L140](../../src/data_preprocessing/mri_preprocessing.py#L140) (and `extract_zip.sh:1`) vs the rest | Preprocessed `.nii.gz` and the unzipped raw MRIs land where downstream stages do **not** look. Inferred latent bug — the author moved data between runs. | Pick **one** root, put all data under it, and edit the preprocessing `__main__` + [extract_zip.sh](../../src/utils/extract_zip.sh) to match. |
| Third root `/home/lucasthim1/...` baked into defaults | [utils.py#L71](../../src/utils/utils.py#L71) (`load_reference_table` default), [base_mri.py#L78](../../src/utils/base_mri.py#L78) (ANTs/NiftyReg) | `load_reference_table` defaults to `/home/lucasthim1/mmml-alzheimer-diagnosis/data/mri/reference/MRI_MPRAGE.csv`; `set_env_variables` sets bogus `ANTSPATH`/`NIFTYREG_INSTALL` `PATH` entries on any other machine. NiftyReg is never actually called; ANTs is used via the `antspyx` package, so these env vars are likely vestigial. | Pass explicit paths; comment out `set_env_variables()` calls or fix the paths to your machine. |
| `set_env_variables` hardcodes Linux ANTs/NiftyReg install paths | [base_mri.py#L78](../../src/utils/base_mri.py#L78) (through `:81`) | `ANTSPATH=/home/lucasthim1/ants/ants_install/bin`, `NIFTYREG_INSTALL=/home/lucasthim1/niftyreg/niftyreg_install` — machine-specific; silently wrong elsewhere. | Edit or skip; ANTs runs through `antspyx` regardless. |
...
### 5.2 `requirements.txt` is badly incomplete

The committed [requirements.txt](../../requirements.txt) lists only `numpy, pandas, matplotlib, sklearn, torch, antspyx` — and `sklearn` is the **deprecated stub package name** (the real distribution is `scikit-learn`; listing `sklearn` may now hard-error on install).
...
| Issue | Location | Impact | Suggested fix |
|---|---|---|---|
| 8 packages missing; deprecated `sklearn` name; everything unpinned | [requirements.txt](../../requirements.txt) | Installing from `requirements.txt` alone will **not** let the explanation, ensemble, AutoML, or skull-strip code run. Expect version conflicts, especially `deepbrain`+`tensorflow` (TF1-era, hardest to install today), `pycaret`, `antspyx`, `torch`/`torchvision`. | Install the true set above. Recommend **two envs**: one TF1/deepbrain env just for skull-strip preprocessing, one modern env for training/eval. The original dev conda env was named `alzheimer` ([src/.vscode/settings.json](../../src/.vscode/settings.json)). |
...
|---|---|---|---|
| Atlas intensity thresholds `(0.05545412003993988, 92.05744171142578)` | [mri_standardize.py#L69](../../src/data_preprocessing/mri_standardize.py#L69) | The 0.02/99.8 percentiles of the **specific** `atlas_t1.nii`; standardization rescales every scan into this range. | If you use a **different atlas**, these constants are wrong — recompute via `get_atlas_thresholds(atlas_path=...)`. See [mri-preprocessing.md](../data/mri-preprocessing.md). |
