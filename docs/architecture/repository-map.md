...
| [ensemble_preprocessing.py](../../src/data_preprocessing/ensemble_preprocessing.py) | **2026 rewrite:** builds the ensemble reference from `COGNITIVE_DATA_PREPROCESSED.csv` alone (single diagnosis source). Sets `MACRO_GROUP = DIAGNOSIS`, `CONFLICT_DIAGNOSIS = False`, drops 3 hardcoded missing MRIs `[293688, 274525, 280596]`, optional `HAS_PREPROCESSED_MRI` flag → `PREPROCESSED_ENSEMBLE_REFERENCE.csv`. (pre-2026: cognitive × MRI merge with a real conflict check.) | implemented |
